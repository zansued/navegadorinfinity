...
def register_windows_protocol():
    ...
    try:
        ...
        if getattr(sys, 'frozen', False):
            ...
            cmd_value = f'"{sys.executable}" "{os.path.abspath(__file__)}" "%1"'
            
        key_path = r"Software\Classes\infinity"
        with winreg.CreateKey(winreg.HKEY_CURRENT_USER, key_path) as key:
            winreg.SetValueEx(key, "", 0, winreg.REG_SZ, "URL:Infinity Protocol")
        ...
        print(f"Erro ao registrar protocolo Windows: {e}")

if __name__ == "__main__":
    # Registra o protocolo Windows para inicialização a partir do navegador
    ...
    # Tratamento de argumentos do protocolo customizado: infinity://
    # Exemplo: infinity://open?key=Ninja%20Hub ou infinity://open/Ninja%20Hub
    for arg in sys.argv:
        if arg.startswith("infinity://"):
            key = None
            if "key=" in arg:
                try:
                    parts = arg.split("key=")
                    if len(parts) > 1:
                        key = urllib.parse.unquote(parts[1].split("&")[0])
                except Exception:
                    pass
            elif arg.startswith("infinity://open/"):
                key = urllib.parse.unquote(arg[len("infinity://open/"):])
            
            if key:
                key = key.rstrip("/").rstrip("\\")
                profiles = fetch_profiles_from_supabase()
                if not profiles:
                    config_path = os.path.join(CURRENT_DIR, CONFIG_FILE)
                    if os.path.exists(config_path):
                        with open(config_path, "r", encoding="utf-8-sig") as file:
                ...
                if profiles and key in profiles:
                    run_profile_headless(key, is_client=not has_admin_key, no_ext=False)
            sys.exit(0)
